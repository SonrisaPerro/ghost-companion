...
_Last updated: 2026-07-02 (v1.3.2). Read this first when resuming work on this tool._
...
- **Shipped:** `v1.0.0`–`v1.3.2` all live (published). **`v1.3.2` (2026-07-02) is
  the current latest** = three bug fixes (see "Recent commits"). `v1.0.6` = sustainable rotation pipeline
...
6. **All releases through v1.3.2 are published + live — nothing pending.** Auto-publish
...
## Recent commits
- **`v1.3.2`** (2026-07-02, live) — three bug fixes bundled together.
  1. Eververse chip now shows the count of user-tracked ornaments in the shop (e.g.
     "1 TRACKED ORNAMENT IN SHOP"), not the full 27-item server-curated list size.
  2. Clicking items/weapons in the WEEK tab now switches to the SCAN tab so the result
     is visible (previously stayed on WEEK and appeared broken).
  3. Escape key and the × clear button now dismiss the scan result and return to the
     My Hunts panel (previously only cleared the input text, leaving the result on screen).
